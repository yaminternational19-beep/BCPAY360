...
export const createLeaveType = async (req, res) => {
  try {
    ...
    const [existing] = await db.query(
      `SELECT id FROM leave_types WHERE company_id = ? AND leave_code = ? AND leave_year = ?`,
      [company_id, leave_code, new Date().getFullYear()]
    );
...
export const deleteLeaveType = async (req, res) => {
  try {
    ...
    await db.query(
      `DELETE FROM leave_types WHERE id = ? AND company_id = ?`,
      [id, company_id]
    );
