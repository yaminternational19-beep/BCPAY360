  ...
  try {
    ...
    const [tickets] = await db.query(
      ...
      SELECT
        ...
        e.full_name,
        e.email,
        ep.profile_photo_url AS profile_photo_path, -- FIXED column name
        cst.branch_id,
        cst.category,
        cst.status,
        cst.created_at
    ...
    const formattedTickets = await Promise.all(
      tickets.map(async (ticket) => {
        let photoKey = ticket.profile_photo_path; // aliases from profile_photo_url
        ...
        if (photoKey && photoKey.startsWith("http")) {
          try {
            ...
            photoKey = ticket.profile_photo_path;
          }
        }
        ...
        return {
          ...
          profile_image_url: photoKey ? await getS3SignedUrl(photoKey) : null,
